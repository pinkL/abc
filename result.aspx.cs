   ...
   SqlConnection conn;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
      ...
      //  TxtSearch.Attributes.Add("onblur", "if(this.value==''){this.value='请输入职位或公司名称'}");
        ...
        if (Session["sousuox"] != null)
        {
            string a = Session["sousuox"].ToString();
            str = "select pos_id,pos_name,com_name,property,time from position where pos_name like '%" + a + "%' or com_name like '%" + a + "%' order by pos_id desc";
            getGoods(str);
        ...
        else
        {
            ...
            str = "select pos_id,pos_name,com_name,property,time from position where property='兼职' order by pos_id desc";
            getGoods(str);
        }
    }
    ...
    protected void lbtbeijing_Click(object sender, EventArgs e)
    {
            ...
            str = "select pos_id,pos_name,com_name,property,time from position where property='兼职'  and area like '%北京%' order by pos_id desc";
            getGoods(str);
    ...
    {
        str = "select pos_id,pos_name,com_name,property,time from position where property='兼职'  and pos_type like '%计算机%' order by pos_id desc";
        getGoods(str);
    }
