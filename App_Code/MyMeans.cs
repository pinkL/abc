...
public class MyMeans
{
    public MyMeans()
    {
        ...
        public static string M_str_sqlcon = "server=LUSHAN-PC\\SQLEXPRESS;database=part-time-job;uid=sa;pwd=1995217ls";
        ...
        /// <param name="M_str_table">表名</param>
        /// <returns>返回DataSet对象</returns>
       /* public Boolean dbUpdate(string SQLstr, string tableName,DataGridView dgv)
        {
            getcon();
            SqlDataAdapter SQLda = new SqlDataAdapter(SQLstr, My_con);  //创建一个SqlDataAdapter对象，并获取指定数据表的信息
            DataSet My_DataSet = new DataSet(); //创建DataSet对象
            SQLda.Fill(My_DataSet, tableName);
            DataTable dtUpdate=My_DataSet.Tables[tableName];
            dtUpdate.Rows.Clear();
            DataTable dtShow = (DataTable)dgv.DataSource;
